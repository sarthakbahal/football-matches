...
const Matches = ({data}:{data:matchestype}) => {
    ...
    <div className='grid grid-cols-3'>
        ...
        <div className='px-2 m-auto flex justify-center items-center bg-gray-400 rounded-md'>
            {data?.status == 'FINISHED' ? (
                <p className='py-1 text-slate-400 text-xs'>{data?.score?.fullTime?.home} : {data?.score?.fullTime?.away}</p>
            ) : (
                <p className='py-1 text-slate-400 text-xs'>{getDate}</p>
            )}
